...
# 数据处理与数据准备，SEU-Gearbox数据集 (最后一列是标签)
root_path = 'SEU_x_y_20-0.csv'
...
for c in range(num_class):
    c_data = data[data.iloc[:, -1] == c].values[:3000,:-1]
    c_data = expand_data_np(c_data, win_size=win_size)
    x_data_list.append(c_data)
    y_data_list.append(c * np.ones((c_data.shape[0])).astype(int))

x_data = np.concatenate(x_data_list, axis=0)
y_data = np.concatenate(y_data_list, axis=0)
print("Shape of dataset:", x_data.shape, y_data.shape)
...
print(x_data.shape[0], y_data.shape[0], y_noisy.shape[0])
...
save_path = 'model/warm_noi_att_te_' + noise_type + '_' + str(noise_rate) + '.pth'
print("----------Warm up-------------")
...
save_path = 'model/fine_noi_att_te_' + noise_type + '_' + str(noise_rate) + '.pth'
print("----------LNL training--------")
...
load_path = 'model/fine_noi_att_te_' + noise_type + '_' + str(noise_rate) + '.pth'
